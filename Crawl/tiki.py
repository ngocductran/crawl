...
#TF-IDF
# truyền api + page
page_url="https://tiki.vn/api/v2/products?limit=10&include=advertisement&aggregations=1&trackity_id=754ce305-af8e-5347-e963-7cd559144973&category=1789&page=1&src=c.1789.hamburger_menu_fly_out_banner&urlKey=dien-thoai-may-tinh-bang"
...
def crawl_product_id():
  ...
  while( i < 10) :
    ...
    response = requests.request(
      ...
      "GET", page_url,headers=headers,data=payload,params=params
      )
    print(response)
    print("page",i)
...
def crawl_product(product_list):
  product_detail_list = np .array(
    [['id','ten','gia','review_count', 'phanloai','thuonghieu','sku','mota','giagoc','giadiscount','discount_rate','point','url_image','url_path','current_seller']])
  ...
  for product_id in product_list:
    ...
    if(response.status_code == 200):
      ...
      print("\t Band:",thuonghieu)
      print("\t sku:",sku)
      print("\t Giagoc:",giagoc)
      print("\t Giagiam:",giadiscount)
      print("\t % giam:",discount_rate)
      print("\t Sao:",point)
      print("\t URL_img:", url_image)
      print("\t URL:", url_path)
      
      mota = y['description']
      #print("t", mota)
      current_seller = y['current_seller']['name']
     
    product_detail_list = np .append(
      product_detail_list,[[id,ten,gia,review_count, phanloai,thuonghieu,sku, mota,giagoc,giadiscount,discount_rate,point,url_image,url_path, current_seller]],axis=0)
         #  vừa craw vừa lưu
    conn= pymysql.connect(host="localhost", user="root", passwd="", db="tiki3")                          
    myCur = conn.cursor()
    sql= "INSERT INTO product(id, ten, gia, review_count, phanloai, thuonghieu, sku, giagoc, giadiscount, discount_rate, point, url_image, url_path, current_seller, mota,price)VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    val= (id,ten,gia,review_count, phanloai,thuonghieu,sku,giagoc,giadiscount,discount_rate,point,url_image,url_path, current_seller, mota, gia)
